...
exec sudo -E chrt -f 95 taskset -c "${RU_CORES}" env \
  ...
  VRTSIM_CATB_STATS="${VRTSIM_CATB_STATS:-}" \
  OAI_XRAN_CAT="${OAI_XRAN_CAT:-}" \
  VRTSIM_CATB_REF_SYMS="${VRTSIM_CATB_REF_SYMS:-}" \
  VRTSIM_CATB_UL="${VRTSIM_CATB_UL:-}" \
  VRTSIM_UE_SPEED_KMH="${VRTSIM_UE_SPEED_KMH:-}" \
  OAI_CATB_BFW_RX="${OAI_CATB_BFW_RX:-}" \
  ORU_PRACH_SAMPLE_SHIFT="${ORU_PRACH_SAMPLE_SHIFT:-}" \
  VRTSIM_UL_NOISE_STD="${VRTSIM_UL_NOISE_STD:-}" \
  VRTSIM_RX_NOISE_SIGMA="${VRTSIM_RX_NOISE_SIGMA:-}" \
