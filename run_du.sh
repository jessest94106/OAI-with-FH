...
exec sudo -E chrt -f 70 taskset -c "${DU_CORES}" env \
  ...
  OAI_CATB_UL_RX="${OAI_CATB_UL_RX:-}" \
  VRTSIM_CATB_REF_SYMS="${VRTSIM_CATB_REF_SYMS:-}" \
  OAI_CATB_DELAY_SLOTS="${OAI_CATB_DELAY_SLOTS:-}" \
  OAI_CATB_SU_BFW="${OAI_CATB_SU_BFW:-}" \
  OAI_CATB_PUB_IRC="${OAI_CATB_PUB_IRC:-}" \
  OAI_CATB_WSYNTH="${OAI_CATB_WSYNTH:-}" \
