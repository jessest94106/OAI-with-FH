...
build_chanmod() {
  ...
  if [[ "${BW_PRB}" != "0" ]]; then
    ...
    # RIV encoding (38.214, N=275): two-branch — the simple 275*(L-1) form is only valid for
    # L-1 <= 137. At 273 PRB it would compute 74800 (garbage); correct is 275*(275-273+1)+274 = 1099.
    local riv
    if (( BW_PRB - 1 <= 137 )); then
      riv=$(( 275 * (BW_PRB - 1) ))
    else
      riv=$(( 275 * (275 - BW_PRB + 1) + 274 ))
    fi
    ...
    # PRACH-FREQ FIX (2026-06-09): the RU extracts the PRACH slice via its OWN prach_msg1_start —
    # it must track the DU's centered placement (BW/2-6) or it reads PRB 0 while the UE transmits
    # at PRB 19/47 -> gNB PRACH energy 0.0 dB -> RAR fails (the historical DU/RU prach-freq mismatch).
    perl -0pi -e 's/(prach_msg1_start\s*=\s*)\d+/${1}'"${prach_start}"'/g;' "${RU_CONF}"
  ...
  if [[ "${CHANMOD}" != "1" ]]; then
    # WEDGE FIX #2 (2026-06-09): the vrtsim server ring clock MUST get the same dilation as the
    # xran side or 100% of DL IQ writes land TOO_LATE and are silently dropped (UE never syncs).
    # vrtsim.timescale defaults to 1.0 -> always forward it, chanmod or not.
    # VRTSIM_TX_ADVANCE: writer's lead (samples) over the ring clock. Default 4096 = 266 µs sim at
    # 15.36 Msps (24 PRB) but only 66 µs at 61.44 Msps (106 PRB) -> wide-BW writes land TOO_LATE and
    # are silently dropped (106 PRACH arrived as 2 samples/symbol). Scale it with sample rate.
    export VRTSIM_RU_EXTRA_ARGS="--vrtsim.timescale ${VRTSIM_TIMESCALE:-1.0} --vrtsim.tx-sample-advance ${VRTSIM_TX_ADVANCE:-4096}"
    export VRTSIM_UE_BASE_ARGS="--vrtsim.timescale ${VRTSIM_TIMESCALE:-1.0} --vrtsim.tx-sample-advance ${VRTSIM_TX_ADVANCE:-4096}"
    export VRTSIM_UE_EXTRA_ARGS="${VRTSIM_UE_BASE_ARGS}"
    # A multi-antenna UE needs the server-side ue_config + client RX-antenna count even with
    # chanmod OFF (else server defaults 1x1 -> "Server expects UE 0 to have 1 TX antennas" abort).
    if [[ "${UE_NB_ANT_TX}" != "1" || "${UE_NB_ANT_RX}" != "1" ]]; then
      local cm="${BASE_DIR}/channelmod_sweep.conf"
      cat > "${cm}" <<EOF
# Auto-generated by sweep_iq_width.sh (CHANMOD=0, multi-antenna): vrtsim ue_config only.
vrtsim = {
  ue_config = (
...
EOF
      ...
      export VRTSIM_RU_EXTRA_ARGS="${VRTSIM_RU_EXTRA_ARGS} --vrtsim.client-num-rx-antennas ${UE_NB_ANT_RX}"
      export UE_NB_ANT_TX="${nbrx}" UE_NB_ANT_RX="${nbtx}"
    fi
    return 0
  fi
...
EOF
  ...
  export VRTSIM_RU_EXTRA_ARGS="--vrtsim.chanmod 1 --vrtsim.timescale ${VRTSIM_TIMESCALE} --vrtsim.client-num-rx-antennas ${UE_NB_ANT_RX} --vrtsim.tx-sample-advance ${VRTSIM_TX_ADVANCE:-4096}"
  ...
  export VRTSIM_UE_BASE_ARGS="--vrtsim.chanmod 1 --vrtsim.timescale ${VRTSIM_TIMESCALE} --vrtsim.tx-sample-advance ${VRTSIM_TX_ADVANCE:-4096}"
  export VRTSIM_UE_EXTRA_ARGS="${VRTSIM_UE_BASE_ARGS}"
  export UE_NB_ANT_TX="${nbrx}" UE_NB_ANT_RX="${nbtx}"
...
stop_stack() {
  ...
  # RU leaks ~8192 hugepages (16 GB) per run as /dev/hugepages/rumap_*; rm -f silently fails on
  # hugepage files -> accumulation -> DPDK starvation -> FH real-time race by the 3rd-4th width.
  sudo find /dev/hugepages -type f -delete 2>/dev/null || true
...
run_trial() {
  ...
  # FRESH shared epoch per trial (2026-06-09): DU+RU must anchor dilated clocks to the SAME,
  # CURRENT wall time. A single epoch exported for a multi-width sweep goes stale leg by leg
  # (the iq10 TTI-skip storm confound). RU and DU inherit this via the environment.
